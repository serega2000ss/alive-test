...
RSpec.feature "Testing Production brand URL: #{production_url}", type: :feature do
  ...
  it 'Visit production /brand/1898brand page' do
  ...
  it 'Visit production /1898/terms page' do
    response = RequestService.send_prod_request("#{production_url}/1898/terms")
    expect(response.code).to eq('200')
    expect(response.body).to have_content('Information about us')
  end

  it 'Visit production /1898/claims page' do
  ...
  it 'Visit production /1898/privacy page' do
  ...
  it 'Visit production /1898/contact page' do
